...
Semantic search interface powered by a locally-hosted Mistral model via Ollama. Surfaces relevant passages with source citations. Operates within a defined persona: precise, dry, comfortable returning nothing when nothing genuinely matches.

### A.R.C.H.I.V.E. — Aggregated Reference Collection: Housed in an Indexed Vault for Evidence
Browsable document library with filtering by document type (BOOK / ARTICLE / DOCUMENT), full-text search by title and author, and inline PDF viewing and download.

### I.N.G.E.S.T. — Intake, Normalization & Generation of Embedded Source Text
Document processing pipeline and upload interface. Accepts PDF, DOCX, and PPTX files. Extracts text via PyMuPDF, chunks with NLTK sentence-aware splitting, generates embeddings via Sentence Transformers, and stores vectors in ChromaDB. Metadata is extracted automatically and confirmed by the user before indexing.
...
- **Tested on:** Raspberry Pi 5, 8GB RAM, Raspberry Pi OS Bookworm 64-bit — sufficient for indexing and retrieval at modest scale, and a deliberate choice for validating the architecture on constrained hardware.
...
```bash
git clone https://github.com/yourusername/project-eris.git
cd project-eris
